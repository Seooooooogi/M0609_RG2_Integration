...
class RobotController(Node):
    ...
    def robot_control(self):
        CLASS_MAP = {"0": "drill", "1": "hammer", "2": "pliers", "3": "screwdriver", "4": "wrench"}
        print(f"Classes: {CLASS_MAP}")
        raw = input("Pick target(s) by number (space-separated, e.g. '1 3'): ").strip()
        if not raw:
            self.get_logger().warn("No input provided")
            return
        target_list = []
        for token in raw.split():
            if token in CLASS_MAP:
                target_list.append(CLASS_MAP[token])
            else:
                self.get_logger().warn(f"Unknown class number: {token}")
    ...
    def pick_and_place_target(self, target_pos):
        movel(target_pos, vel=LINEAR_VELOCITY, acc=LINEAR_ACC)
        mwait()
